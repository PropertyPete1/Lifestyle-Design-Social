...
import { fetchCompetitorCaptions, extractCaptionPatterns, fetchInstagramCompetitorPosts } from './fetchCompetitorCaptions';
 ...
 * Tests smart caption generation with competitor patterns, SEO optimization, and platform specificity
 */
...
export async function testPhase4(): Promise<void> {
...
async function testYouTubeCompetitorScraping(): Promise<TestResult> {
  try {
    const competitors = await fetchCompetitorCaptions();
    
    if (competitors.length >= 5) {
      const titles = competitors.map(c => c.title);
      const hasHooks = titles.some(title => 
        title.includes('WON\'T believe') || 
...
async function testInstagramCompetitorScraping(): Promise<TestResult> {
  try {
    const posts = await fetchInstagramCompetitorPosts();
    
    if (posts.length >= 5) {
      const hasEngagementHooks = posts.some(post => 
        post.caption.includes('Save this') || 
...
async function testPatternExtraction(): Promise<TestResult> {
  try {
...
async function testSEOKeywordIntegration(): Promise<TestResult> {
  try {
    const keywords = await getTopTrendingKeywords(10);
    ...
    const realEstateTerms = keywords.filter(k => 
...
async function testYouTubeCaptionGeneration(openaiApiKey: string): Promise<TestResult> {
  try {
    const testContent = {
    ...
    const result = await prepareSmartCaption(testContent, openaiApiKey, 'youtube');
...
async function testInstagramCaptionGeneration(openaiApiKey: string): Promise<TestResult> {
  try {
    const testContent = {
    ...
    const result = await prepareSmartCaption(testContent, openaiApiKey, 'instagram');
    
    // Check for Instagram-specific elements
    const hasInstagramElements = 
      result.versionA.description.toLowerCase().includes('save this') ||
...
async function testCaptionValidation(openaiApiKey: string): Promise<TestResult> {
  try {
    const testContent = {
    ...
    const noPrices = allVersions.every(v => 
      !v.title.includes('$') && 
    ...
    const noDashes = allVersions.every(v => 
      !v.title.includes('-') && 
      !v.description.includes('-')
    );
...
async function testPlatformSpecificFeatures(openaiApiKey: string): Promise<TestResult> {
  try {
    const testContent = {
    ...
    const youtubeResult = await prepareSmartCaption(testContent, openaiApiKey, 'youtube');
    const instagramResult = await prepareSmartCaption(testContent, openaiApiKey, 'instagram');
...
async function testAutoSaveFeature(): Promise<TestResult> {
  try {
...
async function getOpenAIApiKey(): Promise<string | null> {
  try {
    const settingsPath = path.join(process.cwd(), 'backend', 'settings.json');
    const backupPath = path.join(process.cwd(), 'settings.json');
    
    if (fs.existsSync(settingsPath)) {
      const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
      return settings.openaiApiKey || null;
    } else if (fs.existsSync(backupPath)) {
      const settings = JSON.parse(fs.readFileSync(backupPath, 'utf8'));
      return settings.openaiApiKey || null;
    }
