...
router.post('/batch', async (req, res) => {
  try {
    ...
    const results = await batchFinalPolish(videoIds, platform);
...
router.get('/status/:videoId', async (req, res) => {
  try {
    ...
    const status = await getPhase8Status(videoId);
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    dateFilter.setDate(dateFilter.getDate() - parseInt(days as string));
    ...
    const analytics = await VideoStatus.aggregate([
      { $match: matchStage },
      {
        $group: {
          _id: '$platform',
