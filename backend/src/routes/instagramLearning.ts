...
router.post('/sync', authenticateToken, async (req: Request, res: Response) => {
  try {
    await connectToDatabase();
    const { postCount = 50, includeStories: _includeStories = false, includeReels: _includeReels = true, includePosts: _includePosts = true } = req.body;
...
router.post('/generate-caption', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { videoId, prompt: _prompt, tone: _tone = 'professional', includeHashtags = true, maxLength: _maxLength = 2200 } = req.body;
