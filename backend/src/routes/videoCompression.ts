...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const totalSizeSaved = results.reduce(
    ...
    try {
      ...
      const platformResults = Object.entries(results).map(([platform, result]) => ({
        ...
        downloadUrl: result.success
          ? `/api/compression/download/${path.basename(result.compressedPath)}`
          : null,
        warnings: result.warnings,
      }));
