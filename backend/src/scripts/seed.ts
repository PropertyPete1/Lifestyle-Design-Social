  ...
  posting_schedules: [
    {
      ...
      schedule_type: 'daily',
      posting_times: ['09:00', '13:00', '18:00'],
      enabled_days: [1, 2, 3, 4, 5, 6, 7], // All days
    ...
    },
    {
      ...
      enabled_days: [6, 7], // Saturday and Sunday
      category_rotation: ['real-estate', 'cartoon'],
      posts_per_day: 2,
      is_active: true,
    },
...
const seedVideos = async () => {
  ...
  try {
    ...
    for (const videoData of seedData.videos) {
      ...
      if (result.rows.length > 0) {
        logger.info(`Created video: ${videoData.title}`);
      } else {
        logger.info(`Video already exists: ${videoData.title}`);
      }
    }
  } finally {
    client.release();
...
const seedCaptions = async () => {
  ...
  try {
    ...
    for (const captionData of seedData.captions) {
      ...
      if (result.rows.length > 0) {
        logger.info(`Created caption template: ${captionData.template_name}`);
      } else {
        logger.info(`Caption template already exists: ${captionData.template_name}`);
      }
    }
  } finally {
    client.release();
...
const seedHashtags = async () => {
  ...
  try {
    ...
    for (const hashtagData of seedData.hashtags) {
      ...
      if (result.rows.length > 0) {
        logger.info(`Created hashtag: ${hashtagData.hashtag}`);
      } else {
        logger.info(`Hashtag already exists: ${hashtagData.hashtag}`);
      }
    }
  } finally {
    client.release();
...
const seedPostingSchedules = async () => {
  const client = await pool.connect();
  try {
    logger.info('Seeding posting schedules...');
    ...
    for (const scheduleData of seedData.posting_schedules) {
      ...
      if (result.rows.length > 0) {
        logger.info(`Created posting schedule: ${scheduleData.schedule_name}`);
      } else {
        logger.info(`Posting schedule already exists: ${scheduleData.schedule_name}`);
      }
    }
  } finally {
    client.release();
