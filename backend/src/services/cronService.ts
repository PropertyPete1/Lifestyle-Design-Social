...
export class CronService {
  ...
  private async scheduleDailyPosts(): Promise<void> {
    ...
    try {
      ...
      const enabledUsers = await User.find({
        ...
        .select('_id name email autoPostingSettings')
        .lean();
      ...
      for (const user of enabledUsers) {
        try {
          ...
          if (Array.isArray(results) && results.length > 0) {
            ...
            logger.info(
              `Scheduled ${successfulPosts.length} posts for user ${user.name || userId}`
            );
          } else {
            logger.warn(
              `Failed to schedule posts for user ${user.name || userId}: No posts generated`
            );
          }
        } catch (error) {
      ...
      logger.info(
        `Daily post scheduling completed. Total users: ${enabledUsers.length}, Total posts scheduled: ${totalScheduled}`
      );
    } catch (error) {
  ...
  private async generateDailyReports(): Promise<void> {
    ...
    try {
      ...
      logger.info(
        `Daily report generation completed. Total users: ${users.length}, Reports generated: ${reportsGenerated}`
      );
    } catch (error) {
  ...
  private async updateEngagementMetrics(): Promise<void> {
    try {
      ...
      const posts = await PostModel.find({
        ...
        .limit(50)
        .select('_id userId platform externalPostId')
        .lean();
