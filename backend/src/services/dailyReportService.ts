      ...
      const report: DailyReportData = {
        ...
        platforms: {
          instagram: {
            ...
            successful: posts.filter((p) => p.platform === 'instagram' && p.status === 'posted')
              .length,
          ...
          tiktok: {
            ...
            successful: posts.filter((p) => p.platform === 'tiktok' && p.status === 'posted')
              .length,
        ...
        reports.push({
          ...
          platforms: {
            instagram: {
              ...
              successful: posts.filter((p) => p.platform === 'instagram' && p.status === 'posted')
                .length,
              failed: posts.filter((p) => p.platform === 'instagram' && p.status === 'failed')
                .length,
              views: Math.floor(mockEngagement.totalViews * 0.4),
              engagement: Math.floor(mockEngagement.totalLikes * 0.4),
            },
            ...
            youtube: {
              ...
              successful: posts.filter((p) => p.platform === 'youtube' && p.status === 'posted')
                .length,
    ...
    try {
      ...
      if (failedPosts.length > 0) {
        ...
        for (const post of failedPosts) {
          logger.warn(
            `Failed post: ${post._id} - Platform: ${post.platform} - User: ${(post.userId as any).username}`
          );
        }
      }
