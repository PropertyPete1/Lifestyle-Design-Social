...
const AutoPost = () => {
  ...
  const [cameraRollPath, setCameraRollPath] = useState('');
  const [cartoonStatus, setCartoonStatus] = useState('idle');
  const [cartoonMessage, setCartoonMessage] = useState('');
  const [cartoonStats, setCartoonStats] = useState({ totalCartoons: 0, recentCartoons: [] });
  ...
  const enableAutoPosting = async () => {
    try {
      const response = await axios.post('/api/autopost/enable', {
  ...
  const disableAutoPosting = async () => {
    try {
  ...
  const handleSelectCameraRoll = async () => {
        ...
        <div className="mt-8 bg-gray-50 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <button
              onClick={testScan}
              disabled={isScanning}
              className="bg-blue-600 text-white px-4 py-3 rounded-md hover:bg-blue-700 transition-colors disabled:opacity-50"
            >
            ...
            <button
              ...
              className="bg-pink-600 text-white px-4 py-3 rounded-md hover:bg-pink-700 transition-colors disabled:opacity-50"
            >
                      ...
                      <tr key={index}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-black">
                          <span className="px-2 py-1 rounded text-xs bg-blue-100 text-blue-800">
                            📱 Will check for existing caption
                          </span>
                        </td>
                      </tr>
                    ))}
