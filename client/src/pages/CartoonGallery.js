...
const CartoonGallery = () => {
  ...
  const loadVideoBlobs = async (cartoonsData) => {
    const blobs = {};
    const token = localStorage.getItem('token');
    
    for (const cartoon of cartoonsData) {
      try {
        const response = await axios.get(`/api/autopost/cartoons/download/${encodeURIComponent(cartoon.fileName)}`, {
          responseType: 'blob',
          headers: {
  ...
  const downloadCartoon = async (fileName) => {
    try {
      const token = localStorage.getItem('token');
