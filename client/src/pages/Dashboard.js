...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const [statsRes, postsRes, statusRes] = await Promise.all([
  ...
  const StatCard = ({ title, value, change, icon: Icon, color = 'yellow' }) => (
  ...
  const QuickAction = ({ title, description, icon: Icon, href, color = 'yellow' }) => (
