...
const Settings: React.FC = () => {
  ...
  const [newApiKey, setNewApiKey] = useState<NewApiKey>({ name: '', key: '' });
  const [showApiKeyForm, setShowApiKeyForm] = useState<boolean>(false);
  const [platformStatus, setPlatformStatus] = useState<PlatformStatus>({});
  const [testingConnection, setTestingConnection] = useState<TestingConnection>({});
  ...
  const fetchPlatformStatus = async (): Promise<void> => {
    try {
  ...
  const handleAddApiKey = async (): Promise<void> => {
  ...
  const handleConnectPlatform = async (platform: string): Promise<void> => {
    try {
      ...
      alert(`❌ Error connecting to ${platform}: ` + (error as Error).message);
    }
  };

  const handleDisconnectPlatform = async (platform: string): Promise<void> => {
    ...
    try {
      ...
      alert(`❌ Error disconnecting ${platform}: ` + (error as Error).message);
    }
  };

  const handleTestConnection = async (platform: string): Promise<void> => {
    ...
    try {
      ...
      alert(`❌ Error testing ${platform} connection: ` + (error as Error).message);
    }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-8">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <option value="OpenAI">OpenAI (GPT-4) - Enhanced AI Content</option>
                      <option value="Instagram">Instagram Graph API - Live Posting</option>
                      <option value="TikTok">TikTok API - Live Posting</option>
                      <option value="YouTube">YouTube Data API - Live Posting</option>
                      <option value="RunwayML">RunwayML - AI Video Generation</option>
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="space-y-4">
              {[
                { key: 'autoGenerateCaptions' as keyof SettingsData['content'], label: 'Auto-generate captions with AI', icon: '🤖' },
                { key: 'useTrendingHashtags' as keyof SettingsData['content'], label: 'Use trending hashtags', icon: '#️⃣' },
                { key: 'includeLocation' as keyof SettingsData['content'], label: 'Include location tags', icon: '📍' },
                { key: 'watermark' as keyof SettingsData['content'], label: 'Add watermark to videos', icon: '©️' }
              ].map(setting => (
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="space-y-4">
              {[
                { key: 'email' as keyof SettingsData['notifications'], label: 'Email notifications', icon: '📧' },
                { key: 'push' as keyof SettingsData['notifications'], label: 'Push notifications', icon: '📱' },
                { key: 'postSuccess' as keyof SettingsData['notifications'], label: 'Notify on successful posts', icon: '✅' },
                { key: 'postFailure' as keyof SettingsData['notifications'], label: 'Notify on post failures', icon: '❌' },
                { key: 'lowContent' as keyof SettingsData['notifications'], label: 'Notify when content is low', icon: '⚠️' }
              ].map(setting => (
                <div key={setting.key} className="flex items-center space-x-3">
                  <input
                    type="checkbox"
